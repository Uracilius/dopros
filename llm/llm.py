...
class LLM:
    ...
    def improve_kazakh_transcription(
        self,
        text_or_path,
        prompt_path="llm/prompts/improve_transcription_prompt_kz.txt",
    ):
        try:
    ...
    def improve_russian_transcription(
        self,
        text_or_path,
        prompt_path="llm/prompts/improve_transcription_prompt_rus.txt",
    ):
        try:
            transcription = self._smart_text_detect(text_or_path)
    ...
    def summarize(
        self, text_or_path, prompt_path="llm/prompts/summarize_prompt_kz.txt"
    ):
        with open(prompt_path, "r", encoding="utf-8") as prompt_file:
            prompt = prompt_file.read()
