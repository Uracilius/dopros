...
class RecorderThread(QThread):
    ...
    def __init__(
        self, transcriber, llm, chunk_length_s=5, language=Language.RUSSIAN.value
    ):
        super().__init__()
    ...
    def run(self):
        while not self._stop_flag:
            transcription_text = self.transcriber.record_and_transcribe(
        ...
        if self.language == Language.RUSSIAN.value:
            improved_transcription = self.llm.improve_russian_transcription(
                transcription
            )
        else:
            improved_transcription = self.llm.improve_kazakh_transcription(
                transcription
            )
...
class MainWindow(QMainWindow):
    ...
    def start_recording(self):
        ...
        self.recorder_thread = RecorderThread(
            self.transcriber, self.llm, chunk_length_s=5
        )
        self.recorder_thread.transcription_done.connect(self.handle_transcription)
        self.recorder_thread.improved_transcription_done.connect(
            self.handle_improved_transcription
        )
